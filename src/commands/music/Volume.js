...
module.exports.run = (bot, message, args) => {
  ...
  if (!args.length) {
    let embed = new MessageEmbed()
      ...
      .setDescription(`${prefix}${messages.messages.noArgsVolume}`)
    return message.channel.send(embed)
  }
  ...
  if (isNaN(args[0])) {
    let embed = new MessageEmbed()
      .setColor(color)
      .setDescription(messages.messages.isnanVolume)
    return message.channel.send(embed)
  }
  ...
  if (args[0] < 1 || args[0] > 100) {
    let embed = new MessageEmbed()
      .setColor(color)
      .setDescription(messages.messages.invalidVolume)
    return message.channel.send(embed)
  }
